...
export async function POST(request: Request) {
  try {
    ...
    // Get current user's banner to delete later
    const db = getDb();
    const [user] = await db
    ...
    const fileName = generateFileName(userId, file.name, "banners");
    ...
    // Delete old banner if exists and is from R2
    if (user?.bannerUrl) {
      const oldKey = extractKeyFromUrl(user.bannerUrl);
      if (oldKey) {
        try {
